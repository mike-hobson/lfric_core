...
!-----------------------------------------------------------------------------
!> Drives the execution of the conway miniapp.
!>
!> This is a temporary solution until we have a proper driver layer.
!>
module conway_driver_mod
  ...
  use init_conway_mod,            only : init_conway
  ...
  use conway_alg_mod,             only : conway_alg
...
contains
  ...
  subroutine initialise( program_name, modeldb, calendar )
    ...
    call init_conway( mesh_twod, chi, panel_id, modeldb )
  ...
  subroutine step( program_name, modeldb )
    ...
    type( integer_field_type ),    pointer :: current_state, next_state

    depository => modeldb%fields%get_field_collection("depository")
    call depository%get_field("current_state", current_state)
    call depository%get_field("next_state", next_state)

    ! Call an algorithm
    call conway_alg(current_state, next_state)
  ...
  subroutine finalise( program_name, modeldb )
    ...
    type( field_collection_type ), pointer :: depository
    type( integer_field_type ),    pointer :: current_state
    ...
    call log_event( program_name//': app completed', LOG_LEVEL_INFO )
