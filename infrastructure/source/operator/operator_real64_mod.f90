...
module operator_real64_mod
  ...
  !>
   type, public, extends(operator_parent_type) :: operator_real64_type
    private
    !> Allocatable array of kind real64 which holds the values of the operator
  ...
  contains
    ...
    procedure, public :: initialise => operator_real64_initialiser
    !> Function to get a proxy with public pointers to the data in a
    !! operator_real64_type.
    procedure, public :: get_proxy => get_proxy_operator_real64
    !> Deep copy methods
    procedure, private :: operator_real64_type_deep_copy
    procedure, public  :: deep_copy => operator_real64_type_deep_copy
    !> Destroys object
    procedure, public :: operator_final
    ! Finalizer for the object
    final :: operator_real64_destructor
 end type operator_real64_type
 ...
 !>
 type, public, extends(operator_parent_proxy_type) :: operator_real64_proxy_type
    private
  ...
  contains
    final :: operator_real64_proxy_destructor
 end type operator_real64_proxy_type

!______end of type declarations_______________________________________________

contains

  !> Initialise an <code>operator_real64_type</code> object.
  !>
  ...
  !>
  subroutine operator_real64_initialiser( self, fs_to, fs_from )
    ...
    implicit none
    class(operator_real64_type), target, intent(inout) :: self
  ...
  end subroutine operator_real64_initialiser

  !> Function to create a proxy with access to the data in the operator_real64_type.
  !>
  !> @return The proxy type with public pointers to the elements of
  !> operator_real64_type
  type(operator_real64_proxy_type ) function get_proxy_operator_real64(self)
    implicit none
    class(operator_real64_type), target, intent(in)  :: self

    ! Call the routine that initialises the proxy for data held in the parent
    call self%operator_parent_proxy_initialiser(get_proxy_operator_real64)

    get_proxy_operator_real64 % local_stencil  => self % local_stencil
    get_proxy_operator_real64 % ncell_3d       =  self%ncell_3d
  end function get_proxy_operator_real64

  !> @brief Copies the current operator into a new returned object
  !> @return The copies operator type
  function operator_real64_type_deep_copy(self) result(other)
    implicit none
    class(operator_real64_type), intent(in) :: self
    type(operator_real64_type) :: other
    ! make field_vector
  ...
  end function operator_real64_type_deep_copy

  !>@brief Destroys the operator type
  subroutine operator_final(self)
    implicit none
    class(operator_real64_type), intent(inout) :: self
  ...
  subroutine operator_real64_destructor(self)
    implicit none
    type(operator_real64_type), intent(inout) :: self
    call self%destroy_operator_parent()
    call self%operator_final()
  end subroutine operator_real64_destructor

  !>@brief Destroy the operator proxy
  subroutine operator_real64_proxy_destructor(self)
    implicit none
    type(operator_real64_proxy_type) :: self
    call self%destroy_operator_parent_proxy()
    nullify(self%local_stencil)
  end subroutine operator_real64_proxy_destructor

end module operator_real64_mod
