...
contains
  ...
  subroutine coordinate_jacobian(ndf, ngp_h, ngp_v,   &
                                 chi_1, chi_2, chi_3, &
                                 panel_id,            &
                                 basis, diff_basis,   &
                                 jac, dj              )
  ...
  !-------------------------------------------------------------------------------
    ...
    real(kind=r_def),    intent(in) :: basis(ndf,ngp_h,ngp_v)
    ...
    if (spherical_coord_system == spherical_coord_system_xyz) then
      ...
      do j = 1,ngp_v
        do i = 1,ngp_h
          ...
          do df = 1,ndf
            alpha  = alpha  + chi_1(df)*basis(df,i,j)
            beta   = beta   + chi_2(df)*basis(df,i,j)
            radius = radius + chi_3(df)*basis(df,i,j)
          end do
